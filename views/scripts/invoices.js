...
function update_invoice_info(){
    var isValid = invoice_validate($("#invoice_form"));
    if(isValid == false) return;
    var startDate = new Date($('#final_form input[name="contracted_date"]').val());
    var endDate = new Date($('#final_form input[name="delivery_date"]').val());
    alert($('#final_form input[name="contracted_date"]').val());
    if (startDate > endDate){
        $('#final_form input[name="delivery_date"]').insertAfter('<label for="delivery_date" class="redo">يجب ادخال تاريخ الوصول</label>');
        return;
    }
